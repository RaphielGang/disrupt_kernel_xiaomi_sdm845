...
int byte_count_read;
char Out_buff[PAGE_SIZE];

ssize_t fts_i2c_wr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
ssize_t fts_i2c_wr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
	...
	msleep(20);
	ret = fts_readCmd(&pAddress[3], (byte_count - 4), info->cmd_wr_result, byte_count_read);
#ifdef SCRIPTLESS_DEBUG
	printk("%s:DATA READ\n{", __func__);
...
ssize_t fts_i2c_read_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
	int i ;
...
ssize_t fts_i2c_read_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
...
ssize_t fts_i2c_write_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
	return snprintf(buf, PAGE_SIZE, "%s", info->cmd_write_result);
}

ssize_t fts_i2c_write_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct i2c_client *client = to_i2c_client(dev);
	struct fts_ts_info *info = i2c_get_clientdata(client);
...
static DEVICE_ATTR(iread, S_IWUSR | S_IRUSR, fts_show_error, fts_i2c_read_store);
static DEVICE_ATTR(iread_result, S_IRUGO | S_IWUSR, fts_i2c_read_show, fts_store_error);
static DEVICE_ATTR(iwr, S_IRUSR | S_IWUSR, fts_show_error, fts_i2c_wr_store);
static DEVICE_ATTR(iwr_result, S_IRUGO | S_IWUSR, fts_i2c_wr_show, fts_store_error);
static DEVICE_ATTR(iwrite, S_IRUSR | S_IWUSR, fts_show_error, fts_i2c_write_store);
static DEVICE_ATTR(iwrite_result, S_IRUGO | S_IWUSR, fts_i2c_write_show, fts_store_error);
